...
Another is only to include `Query Frontend` and `Prometheus` inside one node, and they query `CeresDB` cluster through a normal client interface. In this case the assembled compute node is kind of "special client" to `CeresDB`.
...
- Embedded `CeresDB`
    ...
    - Cons.
        - Need to customize a `Prometheus` specific interface. 
        - It's better to bind all three components together for simplicity.
- Separated `CeresDB` cluster
    - Pros.
        - `Prometheus` extension is decoupled with `CeresDB` in both develop and deploy aspects.
