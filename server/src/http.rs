        ...
        warp::path!("sql")
            ...
            .and_then(|req, ctx, proxy: Arc<Proxy<Q>>| async move {
                ...
                match result {
                    ...
                    Err(e) => match e {
                        proxy::error::Error::SqlQueryOverTTL { code, msg } => {
                            Err(reject::custom(Error::SqlQueryOverTTL { code, msg }))
                        }
                        _ => Err(reject::custom(Error::Internal {
                            source: Box::new(e),
                        })),
                    },
                }
...
fn error_to_status_code(err: &Error) -> StatusCode {
    match err {
        ...
        Error::SqlQueryOverTTL { code: _, msg: _ } => StatusCode::OK,
    }
}
