...
pub struct PostgresqlHandler<Q> {
    pub(crate) proxy: Arc<Proxy<Q>>,
    pub(crate) timeout: Option<Duration>,
}

#[async_trait]
impl<Q: QueryExecutor + 'static> SimpleQueryHandler for PostgresqlHandler<Q> {
...
impl<Q: QueryExecutor + 'static> PostgresqlHandler<Q> {
...
fn encode_data(encoder: &mut DataRowEncoder, val: Datum) -> PgWireResult<()> {
    match val {
        ...
        Datum::UInt64(v) => encoder.encode_field(&(v as i64)),
        Datum::UInt16(v) => encoder.encode_field(&(v as i16)),
        Datum::UInt8(v) => encoder.encode_field(&(v as i8)),
    }
}
