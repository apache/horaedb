...
impl DiskCache {
    fn try_new(root_dir: String, cap: usize, partition_bits: usize) -> Result<Self> {
        ...
        Ok(Self {
            root_dir,
            meta_cache: Arc::new(PartitionedMutex::try_new_bit_len(
                init_lru,
                partition_bits,
                SeaHasherBuilder {},
            )?),
...
impl DiskCacheStore {
    pub async fn try_new(
        ...
        let meta_cache = PartitionedMutex::try_new_bit_len(
            init_size_lru,
...
mod test {
    ...
    fn test_disk_cache_bytes_crc() {
        let testcases = vec![("abc", 910901175), ("hello horaedb", 4015368565)];
