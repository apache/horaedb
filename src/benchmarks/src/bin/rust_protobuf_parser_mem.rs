...
//! rust-protobuf parser memory bench.

use benchmarks::{
    rust_protobuf_remote_write::WriteRequest,
    ...
    match config.mode.as_str() {
        "sequential" => {
            for _ in 0..config.scale {
                WriteRequest::parse_from_bytes(&config.test_data)?;
            }
        }
        "concurrent" => {
            ...
            for _ in 0..config.scale {
                ...
                let handle = tokio::spawn(async move {
                    let _ = WriteRequest::parse_from_bytes(&data_clone);
                });
                handles.push(handle);
            }
